    ...
    <section class="table">
        ...
        <section v-if="data.length" class="table-body ui-scrollbar">
            <template v-for="(inf,idx) in data">
                ...
                <!-- 下拉框 -->
                <pull-drop v-if="$scopedSlots.drop" 
                    v-show="active[idx]" :key="idx" :idx="idx"></pull-drop>
            </template>
        </section>
    </section>
</template>
...
export default {
    components: {
        ...
        PullDrop: {
            ...
            render(h) {
                // 获取组件中<template slot="drop" scope="idx"></template>的内容
    ...
    watch: {
        data() {
            if (this.$scopedSlots.drop) this.active = {}
        }
    },
    mounted() {
        this.init();
    ...
    methods: {
        ...
        dropPull(idx) {
            if (this.$scopedSlots.drop) this.$set(this.active,idx,!this.active[idx])
            this.$emit('drop', this.active, idx)
        }
    }
}
</script>
