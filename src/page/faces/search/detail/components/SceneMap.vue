    ...
    <dialog-modal>
        <div :class="$style.scene_map">
            ...
            <p :class="$style.button">
                ...
                <button @click="$emit('close')">关&emsp;闭</button>
            </p>
        </div>
    </dialog-modal>
</template>
...
export default {
    ...
    mounted() {
        this.sceneMap = this.sceneData.image
        this.list = this.sceneData.recDTOs || []
        var self = this
        let img = new Image()
        img.src = img_url + this.sceneMap
        img.onload = function () {
             //获取前端展示图片的宽高
            ...
            for (let i = 0; i < self.list.length; i++) {
                let rect = self.list[i].rect.split('|')
                let leftTop = []
                let rightBottom = []
                let rectInfo = []
    ...
    methods: {
        toHere(item) {
            this.$router.push(`/faces/search/detail?id=${item.id}`)
            this.$emit('detail')
        },
        addToLocus() {
